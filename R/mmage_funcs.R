...
run_dynamod_projection <- function(df,   num_phases = 12, num_cycles = 1) {
    ...
    get_tcla <- function(...) {
        ...
        x <- list(...)
        female_lengths <- c(x$femaleJuvenileAge,
                            x$femaleSubAdultAge,
                            x$femaleAdultCullAge)
        male_lengths <- c(x$maleJuvenileAge,
                          x$maleSubAdultAge,
                          x$maleAdultCullAge)
        if (all(female_lengths == sort(female_lengths)) & all(male_lengths == sort(male_lengths))) {
            return(
                as_tibble(
                    mmage::fclass(c(female_lengths, 1),
                                  c(male_lengths, 1))
                ))
        } else {
            return(as_tibble(
    ...
    get_results <- function(...) {
        ...
        if (length(x$proj) > 0) {
            ...
            average_live_weight <-  sum(average_population * c(x$femaleJuvenileLBW, x$femaleSubAdultLBW, x$femaleAdultLBW, x$femaleAdultLBW,

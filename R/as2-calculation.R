...
ggplot(
  exchange_rates %>% filter(area %in% c("Venezuela (Bolivarian Republic of)", "Zimbabwe", "Belarus")),
  aes(x = year, y = value)
) +
  geom_point() +
...
ggplot(
  as2 %>% filter(iso3_code %in% c("VEN", "BLR", "ZWE")) %>% group_by(country, year) %>% summarise(value = sum(value_constant_usd, na.rm = TRUE)),
  aes(x = year, y = value)
) +
  geom_point() +
...
ggplot(as2 %>% group_by(year) %>% summarise(value = sum(value_constant_usd, na.rm = TRUE)), aes(year, value)) +
  geom_point() +

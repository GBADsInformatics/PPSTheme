...
The following set of equations outlines the terms and calculations used to obtain 
the livestock asset value for a single livestock animal type in a single country for one year 
...
```{r}
readRDS(here::here(config$data$codes$faostat$dir, "FAOSTAT_VOP_Items_NonIndigenous_Codes.rds"))  |> 
...
```{r}
readRDS(here::here(config$data$codes$faostat$dir, "FAOSTAT_Livestock_Codes.rds")) |> 
    dplyr::filter(`Domain Code` == "QCL") |> 
    dplyr::distinct(Item, .keep_all = TRUE) |> 
    print_table(caption = "FAOSTAT Production and Livestock Products Livestock (Stock) Item Codes Used")

...
region <- tcfs %>%  group_by(region23, animal) %>% 
  summarise(region_mean = mean(value, na.rm = TRUE)) 

continent <- tcfs %>%  group_by(continent, animal) %>% 
  dplyr::summarise(continent_mean = mean(value, na.rm = TRUE))
...
tcfs <- tcfs %>% 
  ...
  mutate(
    value = if_else(!is.na(value),value, if_else(!is.na(region_mean),region_mean, continent_mean ))
  ) %>% 
  select(-region_mean, -continent_mean)

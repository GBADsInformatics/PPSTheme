...
knitr::opts_chunk$set(
  ...
  fig.width = 18,
  fig.path = here::here('output', 'figures'), # Set the Images and Figures to the Output Directory
...
```{r}
data <- arrow::read_parquet(get_gbads_file(
  params$file_names$value_of_production, 
...
data <- purrr::map(params$file_names,
  ...
  dir = here::here('data', 'FAOSTAT'))) %>% 
  janitor::clean_names() %>% 
  filter(year %in% params$cutoff_year:params$final_year) %>% 
  clean_countries() %>% 
  sanitize_columns())
...
if (!file.exists(crop_df_name)) {
  ...
  arrow::write_parquet(crop_table, file_name("FAOSTAT", tags = c("Crop", "Values")))
...
if (!file.exists(livestock_df_name)) {
  ...
  livestock$fao_prices <- filter(livestock$prices, months_code == 7021)

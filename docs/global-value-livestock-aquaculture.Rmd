...
fao_tcf_files <- here::here("data", "FAOSTAT", "FAOSTAT_LiveWeight_Conversion_Factors.csv")
```
...
```{r load-livestock-data, cache=TRUE, include=FALSE}
  ...
  livestock$yield <- livestock$yield %>%
    mutate(
      animal_en = trimws(str_remove(item, "meat_"))
    ) %>%
  ...
  livestock_df <- livestock_df %>%
    ...
    mutate(
      lbw_kg = case_when(
        item == "stock" && !is.na(carcass_pct) && (carcass_pct < q10_carcass_pct || carcass_pct > q90_carcass_pct) ~ NA, 
        TRUE ~ lbw_kg
      )
    ) %>% 
    mutate(
